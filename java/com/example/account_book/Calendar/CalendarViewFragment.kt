...
class CalendarViewFragment : Fragment() {
    ...
    private fun CreateMonthViewDynamically(YEAR : Int, MONTH : Int){
        ...
        for (i in 1..calendar.getActualMaximum(java.util.Calendar.DATE)) {
            ...
            val orderBy = ""
            val cursor = AccountInfoManager.getInstance(this.requireActivity().applicationContext, "useraccount")
                .query(3, columns, selection, selectionArgs, orderBy)

            if(cursor.moveToFirst() && !cursor.isNull(cursor.getColumnIndex(AccountInfo.TotalMoneyInfo.TOTAL_DATE))){
                cursor.moveToFirst()
                ...
                DateClassList.add(DateClass(Date8 = IntDate8, Milli = calendar.timeInMillis, Income = income,
                    Spend = spend, Total = totalMoney, ExpenseList =
                    arrayListOf(ExpenseClass(Date8 = IntDate8, Milli = calendar.timeInMillis,
                        Inout = "", Amount = 0, Category = "", Payment = "", Content = ""))))
                MonthIncome += income
                MonthSpend += spend
            }
            else{
